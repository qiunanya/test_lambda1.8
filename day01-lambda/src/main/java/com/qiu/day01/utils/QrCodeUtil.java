...
public class QrCodeUtil {
     ...
     */
     public static boolean zxingCodeCreate(String content, String path, Integer size, String logoPath){
         try {
             ...
             //生成二维码存放文件
             File file = new File(path+UuidUtils.getUUID()+System.currentTimeMillis()+".jpg");
             if (!file.exists()) {
                 file.mkdirs();
             }
   ...
   public static BufferedImage getBufferedImage(String content, Integer size, String logoPath){
        // 如果没有图片大小，则默认是250
       if (StringUtils.isEmpty(size) || size<= StatisContant.ZERO){
           size = 250;
       }
       ...
       try {
        // 1. 设置编码字符集
           ...
           map.put(EncodeHintType.MARGIN,StatisContant.ONE);
           // 2.生成二维码
           MultiFormatWriter multiFormatWriter = new MultiFormatWriter();
           BitMatrix bitMatrix = multiFormatWriter.encode(content, BarcodeFormat.QR_CODE, size, size, map);
           ...
           // 4. 将二维码放入缓冲流
            image = new BufferedImage(codeWidth, codeHeight, BufferedImage.TYPE_INT_BGR);
